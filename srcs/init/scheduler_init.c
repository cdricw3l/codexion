...
t_scheduler *scheduler_init(int nb_coder)
{
	...
	memset(schedul->can_compile, 0, sizeof(int) * nb_coder);
	...
	schedul->can_compile_mu  = malloc(sizeof(pthread_cond_t) * nb_coder);
	if (!schedul->can_compile_mu)
	{
		free(schedul->can_compile_mu);
		free(schedul);
		return (NULL);
	}
	i = 0;
